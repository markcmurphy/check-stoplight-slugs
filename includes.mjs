...
async function checkTocSlugs(projectId, branchId) {
  ...
  // const idArray = ['dove', 'cat', 'hat', 'beer']
  const masterArray = await getToc(projectId, 'master');
  // const masterArray = ['dove','bat','cat','beer'];

  let errors = idArray.filter(slug => 
...
checkTocSlugs('cHJqOjIwNjAz', 'filter-test-branch');
